...
export class Repository<T extends Entity> {
  constructor(public readonly dbContext: Model<T, Key>) {}
   ...
   */
  getId(entity: Key): number {
    return entity.id;
  }

  async getByKey(key: Key): Promise<T> {
    return await this.dbContext.get(key);
  }

  async getBySpec(spec: Specification<T>): Promise<T[]> {
    const scan = await spec.apply(this.dbContext);
    const scanResponse = await scan.exec();
    return scanResponse as T[];
  }
